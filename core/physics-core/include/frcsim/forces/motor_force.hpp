...
class MotorForce final : public ForceGenerator {
	public:
		MotorForce(double max_force_n, Vector3 local_direction = Vector3::unitX())
				: max_force_n_(std::max(0.0, max_force_n)), local_direction_(local_direction.normalized()) {}
		...
		void apply(RigidBody& body, double /*dt_s*/) const override {
				...
				const Vector3 world_direction = body.orientation().rotate(local_direction_).normalized();
