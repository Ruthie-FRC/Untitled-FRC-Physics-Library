...
class SpringForce final : public ForceGenerator {
 public:
  SpringForce(const Vector3& anchor_world_m, const Vector3& attach_local_m,
              double rest_length_m, double stiffness_npm, double damping_nspm)
  ...
  void apply(RigidBody& body, double /*dt_s*/) const override {
    ...
    const Vector3 attach_world_m =
        body.position() + body.orientation().rotate(attach_local_m_);
    ...
    const Vector3 point_velocity_mps =
    ...
    const double spring_force_n =
        -stiffness_npm_ * extension_m - damping_nspm_ * relative_speed_mps;
