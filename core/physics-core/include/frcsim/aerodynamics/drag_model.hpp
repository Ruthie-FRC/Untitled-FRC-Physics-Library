...
class DragModel {
	public:
		...
		Vector3::DragForceDetails computeForceDetailed(const Vector3& velocity_mps) const {
				return Vector3::dragForceDetailed(
					velocity_mps, drag_coefficient_, reference_area_m2_, air_density_kgpm3_, linear_drag_coefficient_n_per_mps_);
		}
		...
		Vector3::DragForceDetails computeForceDetailed(const RigidBody& body) const {
				...
				return Vector3::dragForceDetailed(
					velocity_mps, drag_coefficient_, reference_area_m2, air_density_kgpm3_, linear_drag_coefficient_n_per_mps_);
		}
