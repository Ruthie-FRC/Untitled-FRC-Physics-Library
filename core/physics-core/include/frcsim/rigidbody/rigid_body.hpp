...
explicit RigidBody(double mass_kg) {
setMass(mass_kg);
}

void setMass(double mass_kg) {
mass_kg_ = std::max(0.0, mass_kg);
is_static_ = (mass_kg_ <= 0.0);
inv_mass_ = is_static_ ? 0.0 : (1.0 / mass_kg_);
}

double massKg() const { return mass_kg_; }
double invMass() const { return inv_mass_; }

void setStatic(bool is_static) {
is_static_ = is_static;
inv_mass_ = is_static_ ? 0.0 : ((mass_kg_ > 0.0) ? 1.0 / mass_kg_ : 0.0);
}
...
void setLinearVelocity(const Vector3& linear_velocity_mps) { linear_velocity_mps_ = linear_velocity_mps; }
...
const Vector3 effective_gravity = (flags_.enable_gravity) ? gravity_mps2 : Vector3::zero();
const Vector3 linear_accel_mps2 = force_accumulator_n_ * inv_mass_ + effective_gravity;
const Vector3 angular_accel_radps2 = worldInertiaTensorInv() * torque_accumulator_nm_;

switch (method) {
case IntegrationMethod::kExplicitEuler:
Integrator::integrateLinearExplicit(position_m_, linear_velocity_mps_, linear_accel_mps2, dt_s);
break;
case IntegrationMethod::kRK2:
Integrator::integrateLinearRK2(position_m_, linear_velocity_mps_, linear_accel_mps2, dt_s);
break;
case IntegrationMethod::kSemiImplicitEuler:
default:
Integrator::integrateLinear(position_m_, linear_velocity_mps_, linear_accel_mps2, dt_s);
break;
}

Integrator::integrateAngularVelocity(angular_velocity_radps_, angular_accel_radps2, dt_s);
Integrator::integrateAngular(orientation_, angular_velocity_radps_, dt_s);

const double linear_decay = std::max(0.0, 1.0 - linear_damping_per_s * dt_s);
const double angular_decay = std::max(0.0, 1.0 - angular_damping_per_s * dt_s);
